...
public class BluetoothMultiplayerAndroidManager : MonoBehaviour {
    private static readonly string GameObjectName = typeof(BluetoothMultiplayerAndroidManager).ToString();
    ...
    // Fired when request to enabled Bluetooth discoverability failed for some reason
    // (user dismissed the request dialog or an error occured)
    private void onBluetoothDiscoverabilityEnableFailed(string empty) {
        Action handler = onBluetoothDiscoverabilityEnableFailedEvent;
        if (handler != null) handler();
    }

    // Fired when Bluetooth discoverability was enabled
    private void onBluetoothDiscoverabilityEnabled(string discoverabilityDurationString) {
        ...
        Action<int> handler = onBluetoothDiscoverabilityEnabledEvent;
